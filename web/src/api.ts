...
export type VPNProfileCreate = { name: string; server_public_key: string; endpoint: string; allowed_ips: string[]; dns_servers: string[]; persistent_keepalive: number; address_pool_id: string }
...
export type AddressPoolCreate = { name: string; cidr: string; gateway: string; dns_servers: string[] }
...
export type DeviceCommand = { id: string; device_id: string; device_name?: string; device_uuid?: string; type: DeviceCommandType; status: string; idempotency_key: string; payload: Record<string, unknown>; result?: Record<string, unknown>; error_message?: string; created_at: string; delivered_at?: string; acknowledged_at?: string; finished_at?: string; expires_at: string }
...
export async function createVPNProfile(input: VPNProfileCreate) { const { data } = await api.post<VPNProfile>('/vpn-profiles', input); return data }
export async function listAddressPools() { const { data } = await api.get<{ items: AddressPool[] }>('/address-pools'); return data.items }
export async function createAddressPool(input: AddressPoolCreate) { const { data } = await api.post<AddressPool>('/address-pools', input); return data }
...
export async function listDeviceCommands(deviceId: string, limit = 50, offset = 0) { const { data } = await api.get<{ items: DeviceCommand[]; total: number; limit: number; offset: number }>(`/devices/${encodeURIComponent(deviceId)}/commands`, { params: { limit, offset } }); return data }
export async function createDeviceCommand(deviceId: string, type: DeviceCommandType) { const { data } = await api.post<DeviceCommand>(`/devices/${encodeURIComponent(deviceId)}/commands`, { type, idempotency_key: `${type}-${crypto.randomUUID()}`, payload: {}, ttl_seconds: 300 }); return data }
